...
def printlog(message, *args):
    ...
    with open ('logs/'+filename,'a') as f:
        #print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message, *args, file=f)
        output = message
        for arg in args:
            output += str(arg)
        f.write(output + "\n")
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.4 #변경됨
        return target_price
...
def sell_stop_loss():
    try:
        ...
        for i in range(cpBalance.GetHeaderValue(7)):
            ...
            stock_name = cpBalance.GetDataValue(0, i)   # 종목명
            stock_qty = cpBalance.GetDataValue(15, i)   # 수량
            if cpBalance.GetDataValue(11,i) > 1.2 or cpBalance.GetDataValue(11,i) < 0.97:
                cpOrder.SetInputValue(0, "1")         # 1:매도, 2:매수
                cpOrder.SetInputValue(1, acc)         # 계좌번호
                cpOrder.SetInputValue(2, accFlag[0])  # 주식상품 중 첫번째 -> 여기 확인
        ...
        dbgout("sell_stop_loss() -> exception! " + str(ex))

if __name__ == '__main__': 
    try:
        #여기서 시장 상황에 따라 종목을 바꾸자
        #순서대로
        ##KODEX레버리지, KODEX 200선물인버스2X, KODEX 코스닥150레버리지, TIGER 코스닥150선물인버스, KINDEX 골드선물 레버리지
        ##KODEX골드선물인버스, KODEX WTI원유선물, TIGER원유선물인버스, KBSTAR 국채선물10년, KODEX 국채선물인버스
        symbol_list_original = ['A122630', 'A252670', 'A233740', 'A250780', 'A225130',
             'A280940', 'A261220', 'A217770', 'A295000', 'A176950']
        #후보군 정하자
        ##KODEX인버스, KODEX200, KODEX반도체, TIGER미국나스닥100, TIGER미국S&P500
        ##KBSTAR중국본토대형주CSI100, KODEX 2차전지산업, KODEX자동차, TIGER차이나CSI300, KODEX삼성그룹
        symobl_list_nominate = ['A114800', 'A069500','A091160','A133690','A143850',
             'A174360','A305720','A091180','A192090','A102780']
        #최종
        symbol_list = ['A114800','A069500','A122630','A252670','A225130','A280940','A261220',
             'A217770', 'A295000', 'A176950','A091160','A091180','A102780','A305720']
        bought_list = []     # 매수 완료된 종목 리스트
        target_buy_count = 7 # 매수할 종목 수 -> 변경
        buy_percent = 0.13   # 변경
        printlog('check_creon_system() :', check_creon_system())  # 크레온 접속 점검
        stocks = get_stock_balance('ALL')      # 보유한 모든 종목 조회
        total_cash = int(get_current_cash())   # 100% 증거금 주문 가능 금액 조회
        total_cash -= 10000000                 # ETF 필요예수금 제외
        if (total_cash <= 0) {
            dbgout('계좌 예수금이 부족합니다.')
        }
